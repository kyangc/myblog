...
categories: 技术
tags: [面试,算法,链表] 
description: WuChong嫁我！
---

>**本文转载自WuChong的[Blog](http://wuchong.me/blog/2014/03/25/interview-link-questions/)，这里转载仅留归自用，感谢他的总结与归纳。**
...
Node* reverseByLoop(Node *head)
{
    ...
    Node *pre = NULL;
    Node *next = NULL;
    while(head != NULL)//当head为空时结束
    {
        next = head->next;//暂存next
        head->next = pre;//反转该节点的指向
        pre = head;//pre移动到下一个操作节点
        head = next;//head移动到下一个操作节点
    }
    return pre;//此时head为空，pre是新的首指针
}
...
显然，如果从相遇点M开始，p1 再走 n 步的话，还可以再回到相遇点，同时p2以同样的速度从头开始走的话，经过n步，也会达到相遇点M。
...
题目描述：给出两个单向链表的头指针比如h1、h2，判断这两个链表是否相交。这里为了简化问题，我们假设两个链表均不带环。
